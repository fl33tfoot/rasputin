...
class Rasputin(Frame):
    def __init__(self, screen):
        super().__init__(screen, screen.height, screen.width, can_scroll=False, data=default_data, has_border=True,
                         title="R4SPUT1N")
        ...
        # SUBTAB MAIN LAYOUT
        layout_subtabs_main.add_widget(CheckBox("RGB", on_change=self._toggle_RGB, name="RGB"), 0)
        ...
        layout_subtabs_mgmt.add_widget(CheckBox("DELETE", on_change=self._toggle_RGB, name="RGB"), 6)
    ...
    def _play_audio_short(self):
        global audio
        global vol
        cmd = "mpg123 -a 2 -f " + str(vol) + " /home/admin/rasputin/sounds/talk_short" + str(
            random.randint(1, 2)) + ".mp3 &"
        if audio:
            self.log("> " + cmd)
    ...
    def _play_audio_med(self):
        global audio
        global vol
        cmd = "mpg123 -a 2 -f " + str(vol) + " /home/admin/rasputin/sounds/talk_med" + str(
            random.randint(1, 4)) + ".mp3 &"
        if audio:
            self.log("> " + cmd)
            subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    ...
    def _play_audio_long(self):
        global audio
        global vol
        cmd = "mpg123 -a 2 -f " + str(vol) + " /home/admin/rasputin/sounds/talk_long" + str(
            random.randint(1, 2)) + ".mp3 &"
        if audio:
            self.log("> " + cmd)
            subprocess.run(cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
class RasputinMgmt(Frame):
    def __init__(self, screen):
        ...
        layout_subtabs.add_widget(VerticalDivider(), 1)
        layout_subtabs.add_widget(Button("CLEAR CLI", self.clear_log), 2)
        layout_subtabs.add_widget(VerticalDivider(), 3)
